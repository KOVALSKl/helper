...
function DiseasesGroupPage() {

    const [name, setName] = useState<string>('');
    const [editID, setEditID] = useState<number>(-1);
    const [editing, setEditing] = useState<boolean>(false);

    const {data} = useGetDiseasesGroupsQuery();
    const [addGroup] = useAddDiseasesGroupMutation();
    const [updateGroup] = useUpdateDiseasesGroupMutation();
    const [deleteGroup] = useDeleteDiseasesGroupMutation();
                    ...
                    <Card sx={{borderRadius:'20px'}} elevation={3}>
                        <CardContent sx={{display: 'flex', flexDirection:'column', gap:2}}>
                            <Typography variant="h5" color="primary">
                                {editing ? 'Изменение' : 'Добавление'} Группы Болезней
                                {!editing
                                    ? null
...
export default DiseasesGroupPage;