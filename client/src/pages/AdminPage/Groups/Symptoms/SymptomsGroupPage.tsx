...
import {
    useAddSymptomsGroupMutation, useDeleteSymptomsGroupMutation, useGetSymptomsGroupQuery,
...
function SymptomsGroupPage() {

    const [name, setName] = useState<string>('');
    const [editID, setEditID] = useState<number>(-1);
    const [editing, setEditing] = useState<boolean>(false);

    const {data} = useGetSymptomsGroupsQuery();
    const [addGroup] = useAddSymptomsGroupMutation();
    const [updateGroup] = useUpdateSymptomsGroupMutation();
    const [deleteGroup] = useDeleteSymptomsGroupMutation();
                    ...
                    <Card sx={{borderRadius:'20px'}} elevation={3}>
                        <CardContent sx={{display: 'flex', flexDirection:'column', gap:2}}>
                            <Typography variant="h5" color="primary">
                                {editing ? 'Изменение' : 'Добавление'} Группы Симптомов
                                {!editing
                                    ? null
                                    ...
                                    (data && data.length > 0) ? data.map((item:Group) => (
                                        ...
                                        >
                                            ...
                                            <Box sx={{display:'flex', alignItems:'center'}}>
                                                <IconButton onClick={() => editGroup(item.id)}>
...
export default SymptomsGroupPage;